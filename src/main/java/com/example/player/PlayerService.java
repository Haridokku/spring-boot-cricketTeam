...
public class PlayerService implements PlayerRepository {
    ...
    public Player addPlayer(Player player) {
        player.setPlayerId(playerUniqueId);
        team.put(playerUniqueId, player);
        playerUniqueId += 1;
    ...
    public Player getPlayerById(int playerId) {
        Player playerIsExisted = team.get(playerId);
        if (playerIsExisted == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }

        return playerIsExisted;
    }
